...
def apply_move(board, mini_board, move, sign): # To test
...
def select_best_move_id(moves, next_grid, last_move, sign, depth): # Vali, last_moved
    ...
    if best_UCB == 0:
        ...
        print(f"Moves: {[move[1] for move in moves]}", file=sys.stderr, flush=True)
...
def simulation(last_move, sign): # To test
    ...
    if winner:
        ...
        return 1 if winner == sign else -1
    else:
...
def MCTS(last_move, sign='X', depth=0):
    ...
    if moves:
        ...
        # Leaf node ?
            ...
            # - SIMULATION / CNN
            points = simulation(last_move, ('X' if sign == 'O' else 'O'))
...
def parsing():
    ...
    if last_move[0] != -1:
        apply_move(game, mini_game, last_move, 'O')
...
def print_best_move(last_move):
    ...
    for Nsaid in moves:
        ...
        # print(f"Possible move -> {Nsaid[1]}", file=sys.stderr, flush=True)
    ...
    if best_move:
        ...
        apply_move(game, mini_game, best_move, 'X')
